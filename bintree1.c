...
void insert(node ** root, node * child){
    ...
    else insert( child->value <= (*root)->value ? &(*root)->l : 
&(*root)->r , child );  // recursive call

    /*
	that is ->
	
	else if(child->value <= (*root)->value)
	    insert(&(*root)->l, child); 	  // child value is 
minor/equal to current node call insertion on left node
    
	else
	    insert(&(*root)->r, child); 	  // call insertion on 
right node
    */
}
...
node * search(node * root, int value){
    return !root ? NULL : root->value == value ? root : search( value > 
root->value ? root->r : root->l , value );
    /*
	that is -->
	...
	if(root == NULL)
	    ...
	    return root; 		         // first node address 
found
    
	else if(value > root->value)
	    search(root->r,value);	     // resursive search on 
right side of tree

	else
	    search(root->l,value);       // resursive search on left 
side of tree
